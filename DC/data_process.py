'''
    ...
    将md5值与当前类型数据库那么字段进行比对存在则将当前的同名文件其他格式移动到目录‘’，不存在则
...
class Data(object):
    def __init__(self,table,source,describe,file_type,data_type=None):
        self.table = table
        self.source = source
    ...
    def insert_data_to_db(self,name,company,width,height):
        ...
        if self.data_type == None:
            sql = 'insert into %s values ("%s","%s","%s","%s","%s",%s,%s)'%(self.table,name,self.source,self.describe,company,self.file_type,width,height)
        else:
            sql = 'insert into %s values ("%s","%s","%s","%s","%s","%s","%s",%s)'%(self.table,name,self.source,self.describe,company,self.file_type,self.data_type,width,height)
        try:
            self.cur.execute(sql)
            self.conn.commit()
            self.num += 1
    ...
    def run(self,path):
        ...
        for file_path in file_list:
            if os.path.splitext(file_path)[-1] =='.json':
                img_name, width, height, company = self.parse_json_file(file_path)
                if img_name:
                    ...
                    if self.judge_file_to_db(img_md5):
                        if self.insert_data_to_db(img_md5,company,width,height):
...
if __name__ == '__main__':
    path = '../../test'
    da = Data('damage','自采视频抽帧','damage3','json/xml','多边形')
    da.run(path)
